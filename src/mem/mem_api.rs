...
extern "C" {
    fn __vhost_bb_mem_write(base: MBPtrT, addr: MBPtrT, data: *const u8, len: usize) -> usize;
    fn __vhost_bb_mem_read(base: MBPtrT, addr: MBPtrT, data: *mut u8, len: usize) -> usize;
}
...
impl MBShareMem for DPIBlackBoxShareMem {
    fn write(&mut self, addr: MBPtrT, data: &[u8]) -> usize {
        unsafe { __vhost_bb_mem_write(self.base(), addr, data.as_ptr(), data.len()) }
    }
    fn read(&self, addr: MBPtrT, data: &mut [u8]) -> usize {
        unsafe { __vhost_bb_mem_read(self.base(), addr, data.as_mut_ptr(), data.len()) }
    }
}
