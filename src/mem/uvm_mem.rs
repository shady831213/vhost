...
impl MBShareMem for DPIBlackBoxShareMem {
    fn write(&mut self, addr: MBPtrT, data: &[u8]) -> usize {
        extern "C" {
            fn mb_bb_mem_write(name: *const i8, addr: MBPtrT, data: u8);
        }
        let c_str = std::ffi::CString::new(self.name.as_str()).unwrap();
        for (i, d) in data.iter().enumerate() {
            unsafe {
                mb_bb_mem_write(c_str.as_ptr(), (self.offset(addr) + i) as MBPtrT, *d);
            }
        }
        data.len()
    }
    fn read(&self, addr: MBPtrT, data: &mut [u8]) -> usize {
        extern "C" {
            fn mb_bb_mem_read(name: *const i8, addr: MBPtrT, data: *mut u8);
        }
        let c_str = std::ffi::CString::new(self.name.as_str()).unwrap();
